...
class GameManager:
    ...
    async def start_game(self, channel, mode, initiator):
        ...
        if mode == 'اتحداك':
            ...
            await channel.send(f"@{initiator} تم بدء اللعبة في وضع التحدي الفردي! سنبدأ بالأسئلة.")
    ...
    async def ask_next_question(self, channel):
        ...
        self.is_fate_test = self.active_game.mode != 'اتحداك' and random.random() < 0.1  # 10% chance in team and challenge modes
        # Sabotage question only in تيم and تحدي modes
        self.is_sabotage_question = self.active_game.mode != 'اتحداك' and random.random() < 0.1  # 10% chance
        ...
        if self.current_question:
            await channel.send(f"جج انتهى الوقت! الإجابة الصحيحة هي: {self.current_question.answers.split(',')[0]}")
    ...
    async def handle_question_answer(self, message):
        ...
        if (self.active_game.mode != 'اتحداك' and username not in self.registered_players) or \
           (self.active_game.mode == 'اتحداك' and username != list(self.registered_players)[0]):
            return
    ...
    async def end_game_with_results(self, channel):
        ...
        if game_mode == 'اتحداك' and player.score >= 50:
            motivational_messages = [
                ...
                "ذكاء خارق! أنت تستحق كل التقدير! 🧠",
