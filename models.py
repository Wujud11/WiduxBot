...
class Question(db.Model):
    ...
    question_type = Column(String(20), default='normal')  # normal, golden, steal, doom, fate, sabotage
    created_at = Column(DateTime, default=datetime.datetime.utcnow)
    ...
    def check_answer(self, user_answer):
        ...
        # Normalize the user answer (strip whitespace, lowercase)
        normalized_answer = user_answer.strip().lower()
        
        # Get the list of acceptable answers
        answer_list = [answer.strip().lower() for answer in self.answers.split(',')]
        
        # Check if the user's answer matches any acceptable answer
        return normalized_answer in answer_list
...
class GameSession(db.Model):
    ...
    mode = Column(String(20), nullable=False)  # اتحداك، تحدي، تيم
